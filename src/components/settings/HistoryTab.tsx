  ...
  return (
    <div className="space-y-6">
      <Section title={tx("title", "Harness History")}>
        ...
        <div className="mb-4 rounded-lg border border-codezal bg-codezal-panel-2 px-3 py-2.5 text-md text-codezal-mute">
        ...
        <div className="mb-3 space-y-2">
          {(["claude-code", "codex", "opencode", "cursor"] as SelectableHarness[]).map((h) => (
            <label key={h} className="flex items-center gap-2 text-md">
              <input
                type="checkbox"
                checked={picked[h]}
              ...
              {h === "cursor" && (
                <span className="text-md text-codezal-mute">{tx("experimental", "(experimental)")}</span>
              )}
            </label>
          ))}
        </div>
        ...
        {embedProgress && (
          <div className="mt-2 text-md text-codezal-dim">
      ...
      <Section title={tx("searchTitle", "Search")}>
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full rounded-md border border-codezal bg-codezal-input py-2 pl-9 pr-3 text-md text-codezal-text outline-none focus:border-codezal-accent"
          />
        </div>

        {searching && <div className="text-md text-codezal-mute">{tx("searching", "Searching…")}</div>}
        {!searching && query.trim() && hits.length === 0 && (
          <div className="text-md text-codezal-mute">{tx("noHits", "No matching threads.")}</div>
        )}
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="rounded bg-codezal-chip px-1.5 py-0.5 text-md font-medium text-codezal-dim">
                    {HARNESS_LABEL[h.harness] ?? h.harness}
                  </span>
                  <span className="truncate text-md font-medium text-codezal-text">{h.title}</span>
                </div>
                <div className="truncate text-md text-codezal-mute">{h.snippet}</div>
