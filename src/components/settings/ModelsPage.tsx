...
// management; the standalone Sağlayıcılar page was removed.
//
// Layout choices for scale (20 providers × dozens of models each):
//   - Provider rows are collapsed by default; clicking the row reveals models.
//   - Connected providers auto-expand on first render.
...
export function ModelsPage(): React.ReactElement {
  ...
  // disconnected ones live in the picker modal.
  const adapters = useMemo(() => listProviderAdapters(catalog), [catalog])

  // Probe env vars once for the "Ortam" badge. envFallback off → still probe;
  ...
  // that are actually connected — new ones are added via the picker modal.
  ...
  // Default expanded state — connected providers auto-open on first render.
  // useState lazy initializer runs once; explicit toggles by the user persist.
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    const initial: Record<string, boolean> = {}
    for (const p of adapters) {
      if (isConnectedSync(p, settings)) initial[p.id] = true
    }
    return initial
  })
      ...
      <div className="sticky top-0 z-10 -mx-6 -mt-6 flex flex-col gap-2 border-b border-codezal bg-codezal-bg px-6 pb-3 pt-6">
        ...
        <div className="flex items-center justify-between gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md bg-codezal-accent px-3 py-1 text-[11.5px] font-medium text-white hover:bg-codezal-accent/90"
          >
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-6">
          ...
          {groups.length === 0 && (
            <button
              onClick={() => setPicking(true)}
              className="inline-flex items-center gap-1.5 rounded-md bg-codezal-accent px-3 py-1.5 text-[12px] font-medium text-white hover:bg-codezal-accent/90"
            >
              <Plus className="size-3.5" />
              {t("settings.providersPage.addProvider")}
            </button>
            ...
            return (
              ...
              >
                <ProviderHeader
                  ...
                  onConnect={() => setConnecting(g.provider)}
                  onDisconnect={() => void disconnect(g.provider.id)}
                />
                {open && (
                  <ProviderBody
                    provider={g.provider}
...
function ProviderHeader({
  ...
  return (
    ...
    >
      ...
      {connected && (
        <span className="shrink-0 text-[11.5px] text-codezal-mute">
...
function HeaderButton({
  ...
  return (
    <button
      ...
      className={
        kind === "primary"
          ? "shrink-0 rounded-md bg-codezal-accent px-2.5 py-0.5 text-[11.5px] font-medium text-white hover:bg-codezal-accent/90"
          : "shrink-0 rounded-md border border-codezal px-2 py-0.5 text-[11.5px] text-codezal-dim hover:bg-codezal-input hover:text-codezal-text"
      }
    >
      {children}
    </button>
  ...
  return (
    <div className="border-t border-codezal bg-codezal-bg/40 px-3 py-2">
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <div className="text-[11.5px] text-codezal-mute">
          {filteredOut > 0
            ? t("settings.modelsPage.filteredHint", { hidden: String(filteredOut) })
            : ""}
        </div>
        <div className="flex shrink-0 items-center gap-1 text-[11.5px]">
          ...
          return (
            <li key={m} className="flex items-center justify-between gap-3 px-3 py-1.5">
              <div className="flex min-w-0 items-center gap-2">
                <span className="truncate text-[12.5px] text-codezal-text">
                  {displayName}
                </span>
                {recommended.has(m) && (
                  <span
                    title={t("settings.modelsPage.recommendedTooltip")}
                    className="inline-flex items-center gap-1 rounded-full bg-codezal-accent/15 px-1.5 py-0.5 text-[10.5px] font-semibold text-codezal-accent"
                  >
                    <Sparkles className="size-2.5" />
              ...
              <Toggle enabled={isEnabled(m)} onChange={(v) => onModelToggle(m, v)} />
            </li>
          )
        })}
      </ul>
      ...
      <span
        className={
          enabled
            ...
            : "inline-block size-3.5 translate-x-[2px] rounded-full bg-zinc-500 shadow-sm transition-transform dark:bg-zinc-300"
        }
      />
    </button>
  )
