...
// Akış:
//   1. start() → user_code + verification_uri döner. UI bunları gösterir;
//      kullanıcı browser'da github.com/login/device sayfasına gider, kodu girer.
//   2. pollDeviceCode() → token endpoint polling. Authorization pending olduğu
//      sürece interval saniye uyu, sonra token bekle. Token alındıktan sonra
//      Copilot API token exchange yapılır (api.github.com/copilot_internal/v2/token).
import type { OAuthFlow, OAuthStartResult } from "./types"
import type { OAuthCredential } from "../types"

// Public client id — vscode-copilot CLI ile aynı.
