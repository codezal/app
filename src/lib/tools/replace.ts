...
export const BlockAnchorReplacer: Replacer = function* (content, find) {
  ...
  const substr = (startLine: number, endLine: number): string => {
    let matchStartIndex = 0
  ...
  if (candidates.length === 1) {
    ...
    if (similarity >= SINGLE_CANDIDATE_SIMILARITY_THRESHOLD) yield substr(startLine, endLine)
    return
  }
  ...
  if (maxSimilarity >= MULTIPLE_CANDIDATES_SIMILARITY_THRESHOLD && bestMatch) {
    yield substr(bestMatch.startLine, bestMatch.endLine)
  }
}
...
const normalizeUnicodePunct = (text: string): string =>
  text
    .replace(/[‘’‚‛]/g, "'")
    .replace(/[“”„‟]/g, '"')
...
export const UnicodeNormalizedReplacer: Replacer = function* (content, find) {
  ...
  for (let i = 0; i <= lines.length - findLines.length; i++) {
    ...
    if (normalizeUnicodePunct(block) === normalizedFind) yield block
  }
}
