...
import { checkUrlAllowed } from "./network"
...
export async function loadPlugin(plugin: InstalledPlugin): Promise<LoadResult> {
  ...
  if (plugin.fingerprint) {
    try {
      const current = await computeDirFingerprint(root)
      if (current && current !== plugin.fingerprint) {
        warnings.push(
          "integrity uyarısı: installDir içeriği install anındakiyle eşleşmiyor (tamper olası)",
        )
        void appendAudit({
          ts: Date.now(),
          event: "fingerprint-mismatch",
          plugin: plugin.id,
          detail: "installDir fingerprint mismatch",
        })
      }
    } catch (e) {
      warnings.push(`integrity check başarısız: ${(e as Error).message}`)
    }
  }
