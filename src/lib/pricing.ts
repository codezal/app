...
export type Pricing = TierRates & {
  contextOver200k?: TierRates
}
...
const CONTEXT_TIER_THRESHOLD = 200_000

export function costUsd(model: string, u: UsageBreakdown, fallback?: Pricing | null): number {
  const p = pricingFor(model) ?? fallback
  if (!p) return 0
  const tier =
    u.input > CONTEXT_TIER_THRESHOLD ? p.contextOver200k ?? fallback?.contextOver200k : undefined
  const rate: TierRates = tier ?? p
  const million = 1_000_000
  const cacheRead = u.cacheRead ?? 0
  const cacheWrite = u.cacheWrite ?? 0
  const billableInput = Math.max(0, u.input - cacheRead - cacheWrite)
  return (
    (billableInput / million) * rate.inputPerMTok +
    (u.output / million) * rate.outputPerMTok +
    (cacheRead / million) * (rate.cacheReadPerMTok ?? rate.inputPerMTok) +
    (cacheWrite / million) * (rate.cacheWritePerMTok ?? rate.inputPerMTok)
  )
}
